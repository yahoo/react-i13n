...
const reactDistDomPath = fs.realpathSync(resolve.sync('react-dom/dist/react-dom-server.min', resolveOpts));
...
var PureReactComponentWithI13n = createClass({
    autobind: false,
    mixins: [I13nMixin],
    render: function () {
        return createElement('a', this.props, this.props.children);
...
var createLinkChildren = function (Element, count) {
    var links = [];
    for (let i = 0; i < count; i ++) {
        links.push(createElement(Element, {
            key: i,
